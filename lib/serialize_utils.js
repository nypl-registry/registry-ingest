...
var buildMmsTriples = function(mmsItem,defResults,resourceId,collectionRegistryId){
	...
	mmsItem.notes.map(function(note){
		...
		if (noteMap[note.type]){
			...
			triples.push({  subject: uri,  predicate: "skos:note", objectUri: null, objectLiteral :  note,  literalDataType: null, prov: prov  })
		}else{
	...
	if (mmsItem.catnyp){
		triples.push({  subject: uri,  predicate: "dcterms:identifier", objectUri: "urn:bnum:" + mmsItem.catnyp, objectLiteral :  null,  literalDataType: null, prov: prov  })
	}
				...
				if (collection.bNumber){
					...
					shadowcatBib.find({ _id : bNumber}).toArray(function(err, bibs) {
						...
						dereferenceMmsItem(databaseAgents,databaseTerms,databaseSerialized,databaseMmsCaptures,collection,function(defResults){
							...
							var addedAgents = defResults.registryAgents.map(function(a){ return a._id })
							var addedTerms = defResults.registryTerms.map(function(a){ return a._id })

							console.log(addedAgents)

							if (bibs.length>0){

								// bibs[0]['sc:agents'].map(function(a){

								// 	if (defResults.registryAgents.)
