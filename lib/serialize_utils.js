	...
	db.returnCollectionShadowcat("bib",function(err,shadowcatBib,shadowcatDatabase){
		...
		db.returnDb(function(err,databaseConnection){
			...
			cursor.on('data', function(collection) {
				...
				if (collection.matchedArchives || collection._id == 'b50ab6f0-c52b-012f-5986-58d385a7bc34'){
						...
						if (bibs.length>0){
							...
							function(err, results) {
								...
								if (['7','c','s','d'].indexOf(bibLevel) > -1){
										...
										async.eachSeries(containers, function(c, eachCallback) {
											...
											itemCursor.on('end', function(item) {
												...
												relationships.map(function(r){
													...
													console.log({  subject: parentUriLookup[r.s],  predicate: r.p, objectUri: parentUriLookup[r.o], objectLiteral :  null,  literalDataType: null, prov: r.g  })
													if (!parentLookup[r.s] && !parentLookup[r.o]){
														console.log(r)
													}
												})
									...
									console.log("Is a colelction:",collection.title,collection._id)
									...
									//console.log("Is a item:",bibLevel,"   " ,collection.title)
