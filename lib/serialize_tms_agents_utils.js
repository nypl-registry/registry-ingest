...
exports.buildAgentFromTmsAgent = function(tmsAgent){
	...
	if (tmsAgent.id) newAgent.tmsConstituent = tmsAgent.id
					...
					async.eachSeries(checkNames, function(checkName, eachSubCallback) {
						...
						if (!foundSomething){
							serializeUtils.returnAgentByName(checkName, function(existingAgent){
								if (existingAgent){
									foundSomething = { nameTms : checkName, nameAgents: existingAgent.nameControlled, viaf: existingAgent.viaf, registry: existingAgent.registry  }
								}
								eachSubCallback()
							})	
						}else{
					   	...
					   	if (foundSomething){
					   		...
					   		//but we want to add in that we have a TMS consitutent ID
					   		serializeUtils.updateTmsConstituent(foundSomething.registry,agent.id,function(){
					   			eachCallback()
					   		})
						...
						serializeUtils.returnAgentByViaf(viaf,function(existingAgent){
							...
							if (!existingAgent){
								...
								//already in agents, it is okay
						   		//but we want to add in that we have a TMS consitutent ID
						   		serializeUtils.updateTmsConstituent(existingAgent.registry,agent.id,function(){
						   			eachCallback()
						   		})
