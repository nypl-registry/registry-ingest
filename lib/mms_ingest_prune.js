...
exports.pruneIgnoreCollections = function(cb){
	...
	db.returnCollection("mmsItems",function(err,items,databaseItems){

		//get a db cursor to insert on
		db.returnCollection("mmsContainers",function(err,containers,databaseContainers){


			db.returnCollection("mmsCollections",function(err,collections,databaseCollections){
				...
				var deleteStuff = function(){
					...
					if (!uuids[0]){
						...
						cb()
						databaseItems.close()
						databaseContainers.close()
						databaseCollections.close()
						return true
					}
	...
	db.returnCollection("mmsItems",function(err,items,databaseItems){
		...
		db.returnCollection("mmsContainers",function(err,containers,databaseContainers){
			...
			containers.find().toArray(function(err, containerArray) {
				...
				db.returnCollection("mmsCaptures",function(err,captures,databaseCaptures){
