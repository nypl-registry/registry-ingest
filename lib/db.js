...
exports.prepareIngestArchivesCollections = function(cb){
	...
	MongoClient.connect(mongoConnectURL, function(err, db) {
		...
		var collection = db.collection('archivesCollections')
...
exports.prepareIngestArchivesComponents = function(cb){
	...
	MongoClient.connect(mongoConnectURL, function(err, db) {
		...
		var collection = db.collection('archivesComponents')
...
exports.prepareIngestTmsObject = function(cb){
	...
	MongoClient.connect(mongoConnectURL, function(err, db) {
		...
		var collection = db.collection('tmsObjects')
...
exports.prepareViafLookup2 = function(cb){
	...
	MongoClient.connect(mongoConnectURL, function(err, db) {
		...
		var collection = db.collection('viafLookup2')

		//delete any existing data first
		collection.drop(function(err,results){
			if (err){
				if (err.message != 'ns not found'){
					console.log(clc.redBright('viafLookup2 - drop collection'), clc.greenBright(err), console.dir(err))
				}
			}
...
exports.prepareAgents = function(cb){
	...
	MongoClient.connect(mongoConnectURL, function(err, db) {
		...
		var collection = db.collection('agents2')

		//delete any existing data first
		collection.drop(function(err,results){
			if (err){
				if (err.message != 'ns not found'){
					console.log(clc.redBright('prepare agents2 - drop collection'), clc.greenBright(err), console.dir(err))
				}
			}
			...
			collection.createIndex("nameControled", {background: true})
...
exports.exportPDItems = function(cb){



	MongoClient.connect(mongoConnectURL, function(err, db) {

		var collection = (exports.testOverride) ? db.collection('test') : db.collection('mmsItems')
		var fs = require('fs');

		var c = 0
		var cursor = collection.find({ publicDomain : true })
		...
		cursor.on('data', function(doc) {
			...
			db.collection('mmsCollections').find({ _id : doc.collectionUuid }).toArray(function(err, col) {

				var title =""
				if (col[0]) if (col[0].title) title = col[0].title

				doc.collectionTitle = title
				var out = JSON.stringify(doc)
				c++

				fs.appendFile('data/pd_mms_items.json', out + "\n", function (err) {
					cursor.resume()
					console.log(c)
				})
