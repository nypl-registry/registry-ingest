...
exports.prepareTripleStoreResources = function(cb){
	exports.databaseConnectTripleStore(function(){
		...
		//remove the any old stuff
		var collection = exports.databaseTripleStore.collection('resourcesMms')
		console.log("Dropping Table")
		//delete any existing data first
		collection.drop(function(err,results){
			console.log("resourcesMms Table dropped")
			if (err){
				if (err.message != 'ns not found'){
					console.log(clc.redBright('prepare resourcesMms - drop collection'), clc.greenBright(err), console.dir(err))
				}
			}
