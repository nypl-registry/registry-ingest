...
exports.populateShadowcatAgentsBuildQueue = function(cb){
	...
	db.returnCollectionShadowcat("bib",function(err,shadowcatBib){
		...
		queueTimer = setInterval(function(){
			if (exports.shadowCatAgentsQueue.length<20000){
				if (!resumeOn){
					cursor.resume()
					resumeOn = true
				}					
	...
	if (scAgent && !registryAgent){
		...
		if (viaf){
			...
			if (viaf.lcTerm){
				...
				if (newAgent.nameNormalized.indexOf(normal) === -1 && normal !== '') newAgent.nameNormalized.push(normal)
			}
			if (viaf.viafTerm){
		...
		if (!registryAgent.viaf && viaf){
			...
			if (viaf.lcTerm){
				var normal = utils.normalizeAndDiacritics(viaf.lcTerm).trim()
				if (registryAgent.nameNormalized.indexOf(normal) === -1 && normal !== '') registryAgent.nameNormalized.push(normal)
			}
			if (viaf.viafTerm){
				var normal = utils.normalizeAndDiacritics(viaf.viafTerm).trim()
				if (registryAgent.nameNormalized.indexOf(normal) === -1 && normal !== '') registryAgent.nameNormalized.push(normal)
