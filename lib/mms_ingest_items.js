...
exports.ingest = function(cb){
	...
	db.prepareIngestMmsItems(function(){
		...
		exec('wc -l ' + filePath, function (error, results) {
			...
			db.returnCollectionRegistry("mmsItems",function(err,collection){
				...
				file.streamJsonFile(filePath, function(record,recordCb){
					...
					if (xml){
						...
						collection.insert(insert, function(err, result) {	
							...
							if (err){
								errorLib.error("MMS items ingest - error inserting record",err,database)
							}else{
								totalInserted++
							}					
						...
						errorLib.error("MMS items ingest - no/invalid XML for this item ",insert._id,database)						
					}
